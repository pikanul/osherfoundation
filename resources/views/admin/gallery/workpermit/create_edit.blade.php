<form action="{{ $workpermit ? route('admin.gallery.workpermit.update', $workpermit->id) : route('admin.gallery.workpermit.store') }}" method="POST" class="" enctype="multipart/form-data">
    @csrf
    @if ($workpermit)
        @method('put')
    @endif
        ...
        <div class="col-xl-6 col-md-6 col-12 mb-1">
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="name" name="name" placeholder="Enter Name"
                    value="{{ $workpermit ? $workpermit->name : ''}}">
        ...
        <div class="col-xl-6 col-md-6 col-12 mb-1">
            <div class="">
                <label type="button" onclick="upload_select(this)"> Image <br>
                    <input type="text" name="upload_id" id="upload_id" value="{{ $workpermit ? $workpermit->upload_id : 0 }}"
                        class="form-control mb-2" hidden>
                    <img style="max-height: 60px" src="{{ dynamic_asset($workpermit ? $workpermit->upload_id : 0) }}" alt="">
                </label>
            </div>
        </div>
         <div class="col-lg-6">
            ...
            <div class="items_container_image">
                <div class="items_filed_iamge">
                    {{-- items --}}
                    @if($workpermit)
                        @foreach (dynamic_assets($workpermit->upload_ids) as $key => $item)
    ...
    <button class="btn btn-primary waves-effect waves-float waves-light float-right" type="submit">{{ $workpermit ? 'Update' : 'Create' }}</button>
    </div>
</form>