<form action="{{ $category ? route('admin.course.category.update', $category->id) : route('admin.course.category.store') }}"
        ...
        <div class="col-xl-6 col-md-6 col-12 mb-1">
            <div class="">
                <label type="button" onclick="upload_select(this)"> Image <br>
                    <input type="text" name="image" id="image" value="{{ $category ? $category->image : 0 }}"
                        class="form-control mb-2" hidden>
                    <img style="max-height: 60px" src="{{ dynamic_asset($category ? $category->image : 0) }}"
                        alt="">
                </label>
            </div>
        </div>
