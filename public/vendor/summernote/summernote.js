...
var ImageDialog = /*#__PURE__*/function () {
  ...
  ImageDialog_createClass(ImageDialog, [{
    ...
    var body = `
          <div  onclick="upload_select(this)" data-url_place="true">
            <div class="form-group note-form-group note-group-select-from-files">
              <label for="note-dialog-image-file-${this.options.id}" class="note-form-label">
                ${this.lang.image.selectFromFiles}
              </label>
            </div>
            <img src="https://img.icons8.com/ios/50/000000/image.png" alt="Image" style="height:80px;width:120px; padding:5px"/>
            <div class="form-group note-group-image-url">
              <label for="note-dialog-image-url-${this.options.id}" class="note-form-label" hidden>
                ${this.lang.image.url}
              </label>
              <input id="note-dialog-image-url-${this.options.id }"  hidden class="note-image-url form-control note-form-control note-input" type="text"/>
            </div>
            </div>
        `;
...
var dialog = renderer.create('<div class="modal note-modal" aria-hidden="false" tabindex="-1" role="dialog"></div>', function ($node, options) {
  ...
  $node.html(['<div class="modal-dialog">', '<div class="modal-content">', options.title ? '<div class="modal-header flex-sm-row-reverse">' + '<button type="button" class="close" data-dismiss="modal" aria-label="Close" aria-hidden="true">&times;</button>' + '<h4 class="modal-title">' + options.title + '</h4>' + '</div>' : '', '<div class="modal-body">' + options.body + '</div>', options.footer ? '<div class="modal-footer">' + options.footer + '</div>' : '', '</div>', '</div>'].join(''));
});
